...
export function isSettingsSubmoduleRoute(pathname: string) {
  const normalizedPathname = normalizePathname(pathname)

  return getRouteCandidates(APP_ROUTES.configuracoes).some(
    (candidate) => normalizedPathname.startsWith(`${candidate}/`),
  )
}

export function isSettingsRootRoute(pathname: string) {
  return isAppRouteActive(pathname, APP_ROUTES.configuracoes)
}
